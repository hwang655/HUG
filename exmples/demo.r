...
# Zero proportion vs expression level
load('Data/Gierahn2017_raw.rData')
props.nz = colMeans(dat.gierahn != 0)
expression.nz = colSums(dat.gierahn) / colSums(dat.gierahn != 0)
plot(log(expression.nz), props.nz, pch='.', cex=2,
     main='expression proportion vs expression level',
     xlab='log(average nonzero expression)', 
     ylab='proportion of nonzero expression')
